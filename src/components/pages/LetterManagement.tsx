...
const LetterManagement: React.FC<{ setSuccessMsg: (msg: string) => void }> = ({ setSuccessMsg }) => {
  ...
  const [rejectNotification, setRejectNotification] = useState<{show: boolean, message: string, letterSubject: string}>({show: false, message: "", letterSubject: ""});
  ...
  useEffect(() => {
    ...
    axios
      .get("http://localhost:5000/api/letters")
      .then((res: { data: Letter[] }) => {
        const rejected = res.data.filter((letter: Letter) => letter.status === "rejected");
  ...
  const handleApprovePendingLetter = async (id: string) => {
    try {
      await axios.post("http://localhost:5000/api/letters/approve", { letterId: id });
  ...
  return (
    <div className="w-full flex flex-col h-full bg-gray-50">
      ...
      <div className="bg-white border-b border-gray-200 px-6 py-4">
        <h2 className="text-2xl font-bold text-gray-800 mb-2">Letter Management</h2>
        <p className="text-gray-600">Manage and approve pending letters, review rejected letters, and oversee all correspondence</p>
      </div>
        ...
        <div className="fixed top-4 left-1/2 transform -translate-x-1/2 z-50 w-full max-w-md mx-auto">
          <div className="bg-green-600 text-white px-6 py-4 rounded-lg shadow-lg flex items-center gap-3 animate-fade-in">
            ...
            <div>
              ...
              <div className="text-sm opacity-80">Subject: <span className="font-medium">{rejectNotification.letterSubject}</span></div>
            </div>
          </div>
        </div>
      )}
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          ...
          >
            <div className="flex items-center">
              ...
              <div>
                ...
                <p className="text-2xl font-bold text-yellow-900">{pendingLetters.length}</p>
              </div>
            </div>
          </div>
          ...
          >
            <div className="flex items-center">
              ...
              <div>
                <p className="text-sm font-medium text-red-800">Rejected Letters</p>
                <p className="text-2xl font-bold text-red-900">{letters.filter(letter => letter.status === "rejected").length}</p>
              </div>
            </div>
          </div>
          ...
          >
            <div className="flex items-center">
              ...
              <div>
                <p className="text-sm font-medium text-blue-800">Total Letters</p>
                <p className="text-2xl font-bold text-blue-900">{letters.length}</p>
              </div>
            </div>
          </div>
        </div>
          ...
          <div className="flex items-center justify-between">
            <div className="flex items-center">
              ...
              <span className={`px-3 py-1 rounded-full text-sm font-semibold ${
                ...
                {activeFilter === "pending" ? "Pending Letters" : "Rejected Letters"}
              </span>
            </div>
            <button
              onClick={() => setActiveFilter("all")}
      ...
      <div className="flex-1 overflow-y-auto">
        ...
        {pendingLetters.length > 0 && (activeFilter === "all" || activeFilter === "pending") && (
          <div className="bg-gradient-to-r from-yellow-50 to-orange-50 border-b border-yellow-200 p-6">
            <div className="flex items-center justify-between mb-6">
              <div className="flex items-center">
                ...
                <div>
                  <h3 className="text-xl font-bold text-yellow-800">Pending High/Urgent Letters</h3>
                  <p className="text-yellow-600 text-sm">Require admin approval before sending</p>
                </div>
              </div>
            ...
            <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-3">
              {pendingLetters.map((letter) => (
                <div key={letter._id} className="bg-white shadow-lg rounded-xl p-6 border-l-4 border-yellow-500 hover:shadow-xl transition-all duration-200">
                  <div className="flex items-start justify-between mb-4">
                    <h4 className="font-semibold text-gray-800 text-lg leading-tight">{letter.subject}</h4>
                    <span className={`px-2 py-1 rounded-full text-xs font-bold ${
                      letter.priority === 'urgent' ? 'bg-red-100 text-red-800' : 'bg-orange-100 text-orange-800'
                    }`}>
                      {letter.priority}
                    </span>
                  </div>
                  ...
                  <div className="space-y-2 text-sm text-gray-600 mb-4">
                    <div className="flex items-center">
                      ...
                      <span><strong>From:</strong> {letter.fromName || letter.fromEmail}</span>
                    </div>
                    <div className="flex items-center">
                      <svg className="w-4 h-4 mr-2 text-gray-400" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M3 8l7.89 4.26a2 2 0 002.22 0L21 8M5 19h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v10a2 2 0 002 2z" />
                      </svg>
                      <span><strong>To:</strong> {letter.to || letter.toEmail}</span>
                    </div>
                    <div className="flex items-center">
                      <svg className="w-4 h-4 mr-2 text-gray-400" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M8 7V3m8 4V3m-9 8h10M5 21h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v12a2 2 0 002 2z" />
                      </svg>
                      <span><strong>Date:</strong> {getLetterSentDate(letter.createdAt)}</span>
                    </div>
                  </div>
                  ...
                  <div className="flex flex-col gap-2">
                    ...
                    <button
                      ...
                      onClick={() => { setShowRejectModal(letter); setRejectComment(""); }}
                    >
        ...
        {letters.filter(letter => letter.status === "rejected").length > 0 && (activeFilter === "all" || activeFilter === "rejected") && (
          <div className="bg-gradient-to-r from-red-50 to-pink-50 border-b border-red-200 p-6">
            <div className="flex items-center justify-between mb-6">
              <div className="flex items-center">
                ...
                <div>
                  <h3 className="text-xl font-bold text-red-800">Rejected Letters</h3>
                  <p className="text-red-600 text-sm">Letters that were rejected and sent back to sender</p>
                </div>
              </div>
              <span className="bg-red-500 text-white px-3 py-1 rounded-full text-sm font-semibold">
                {letters.filter(letter => letter.status === "rejected").length} rejected
              </span>
            </div>
            
            <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-3">
              {letters.filter(letter => letter.status === "rejected").map((letter) => (
                <div key={letter._id} className="bg-white shadow-lg rounded-xl p-6 border-l-4 border-red-500 hover:shadow-xl transition-all duration-200">
                  <div className="flex items-start justify-between mb-4">
                    <h4 className="font-semibold text-gray-800 text-lg leading-tight">{letter.subject}</h4>
          ...
          <div className="bg-white p-6">
            <div className="flex items-center justify-between mb-6">
              <div className="flex items-center">
                ...
                <div>
                  <h3 className="text-xl font-bold text-gray-800">All Letters</h3>
                  <p className="text-gray-600 text-sm">Complete overview of all correspondence</p>
                </div>
              </div>
            </div>
                ...
                {filteredLetters.map((letter) => (
                  ...
                  >
                    <div className="flex justify-between items-start mb-4">
                      <h4 className="font-semibold text-gray-800 text-lg leading-tight flex-1 pr-2">{letter.subject}</h4>
                      <span
                    ...
                    <div className="space-y-2 text-sm text-gray-600 mb-4">
                      <div className="flex items-center">
                        ...
                        <span><strong>Sender:</strong> {letter.fromName || "Unknown"}</span>
                      </div>
                      <div className="flex items-center">
                        <svg className="w-4 h-4 mr-2 text-gray-400" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                          <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M3 8l7.89 4.26a2 2 0 002.22 0L21 8M5 19h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v10a2 2 0 002 2z" />
                        </svg>
                        <span><strong>Receiver:</strong> {letter.to || "Unknown"}</span>
                      </div>
                      <div className="flex items-center">
                        <svg className="w-4 h-4 mr-2 text-gray-400" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                          <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M13 16h-1v-4h-1m1-4h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z" />
                        </svg>
                        <span><strong>Priority:</strong> {letter.priority}</span>
                      </div>
                    </div>
                    ...
                    <div className="flex flex-wrap gap-2">
                      ...
                      <button
                        className="bg-green-600 text-white px-3 py-1 rounded-lg shadow flex items-center gap-1 hover:bg-green-700 transition-colors"
                      ...
                      <button
                        className="bg-blue-600 text-white px-3 py-1 rounded-lg shadow flex items-center gap-1 hover:bg-blue-700 transition-colors"
                      ...
                      <button
                        className="bg-gray-700 text-white px-3 py-1 rounded-lg shadow flex items-center gap-1 hover:bg-gray-800 transition-colors"
                      ...
                      {letter.attachmentUrl && (
                        ...
                        >
                          <Download className="w-4 h-4" /> Attach
                        </button>
                      )}
                      <button
                        className="bg-purple-600 text-white px-3 py-1 rounded-lg shadow flex items-center gap-1 hover:bg-purple-800 transition-colors"
                      ...
                      {acceptedLetters.includes(letter._id) ? (
                        ...
                        <button
                          className="bg-pink-600 text-white px-3 py-1 rounded-lg shadow flex items-center gap-1 hover:bg-pink-800 transition-colors"
        ...
        >
          {!viewMode ? (
            <div className="p-6 max-h-[80vh] overflow-y-auto">
              <h3 className="text-xl font-bold text-gray-800 mb-4">Letter Details</h3>
              <div className="space-y-3 text-gray-700">
                <div><strong>Subject:</strong> {openLetter.subject}</div>
                <div><strong>To:</strong> {openLetter.to || openLetter.toEmail}</div>
                <div><strong>From:</strong> {openLetter.fromName || openLetter.fromEmail}</div>
                <div><strong>Department:</strong> {openLetter.department}</div>
                <div><strong>Priority:</strong> <span className={`font-bold ${openLetter.priority === 'urgent' ? 'text-red-600' : openLetter.priority === 'high' ? 'text-orange-600' : 'text-blue-600'}`}>{openLetter.priority}</span></div>
                <div><strong>Date:</strong> {getLetterSentDate(openLetter.createdAt)}</div>
            ...
            <div className="p-6 max-h-[90vh] overflow-y-auto">
              <div className="mb-4 flex justify-between items-center">
                <h3 className="text-xl font-bold text-gray-800">Letter Content</h3>
      ...
      {showRejectModal && (
        ...
        >
          <div className="p-6">
            <h3 className="text-xl font-bold text-gray-800 mb-4">Reject & Forward to Sender</h3>
            <div className="mb-4">
              <label className="block text-sm font-medium text-gray-700 mb-1">To (Sender)</label>
              <input
                type="text"
                value={showRejectModal.fromName || showRejectModal.fromEmail || ""}
                readOnly
            ...
            <div className="mb-4">
              <label className="block text-sm font-medium text-gray-700 mb-1">Comment (Reason for Rejection)</label>
              <textarea
                value={rejectComment}
              ...
              <button
                ...
                onClick={async () => {
                  ...
                  try {
                    ...
                    formData.append("subject", `Rejected letter: ${showRejectModal.subject}`);
                    formData.append("from", "admin@system.local"); // Admin email
                    formData.append("to", showRejectModal.fromName || showRejectModal.fromEmail || "");
                    formData.append("department", showRejectModal.department || "");
                    formData.append("priority", showRejectModal.priority || "normal");
                    formData.append("content", rejectComment
                      ? `${rejectComment}\n\n--- Rejected Letter ---\n\n${showRejectModal.content || ""}`
                      : `--- Rejected Letter ---\n\n${showRejectModal.content || ""}`);
                    formData.append("ccEmployees", JSON.stringify({}));
                    formData.append("cc", JSON.stringify([]));
                    
                    await axios.post("http://localhost:5000/api/letters", formData, {
                      headers: {
                    ...
                    setRejectNotification({
                      show: true,
                      message: "Letter has been successfully rejected and forwarded back to the sender with your comments.",
                      letterSubject: showRejectModal.subject
                    });
                    setTimeout(() => setRejectNotification({show: false, message: "", letterSubject: ""}), 5000);
                    
                    // Move letter from pending to rejected category instead of removing
                    setPendingLetters(prev => prev.filter(l => l._id !== showRejectModal._id));
